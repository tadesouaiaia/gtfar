...
class GtfFile:
    def __init__(self, fileHandle, prefix,readlen,filterType=None,gapList=None,mutationList=None,printKEY=True):
        ...
        self.filters = GtfFilters(filterType)
        ...
        if gapList != None:
            self.gapList = True
            self.gapFile = open(gapList)
            self.gapOpen = True
            self.gapLine = self.gapFile.readline().split()
            self.gapChr = self.gapLine[0]
            self.loadGapCands()
...
############################################################################################################################################
    ...
    def loadGapCands(self):
        gapChr,chrStart,chrEnd,gene,geneStart,geneEnd = self.gapLine
        gapCands = dd(list);
        gapCnt = 1
        self.chrGaps = {}
        while self.gapOpen:
            while gapChr == self.gapChr:
                tmpLine = self.gapFile.readline().split()
                while tmpLine == self.gapLine:
                    gapCnt += 1
                    tmpLine = self.gapFile.readline().split()
                    gene=gene.split("|")[0]
                if gapCnt>1:    gapCands[gene].append((int(chrStart),int(chrEnd),gapCnt))
                gapCnt=1
                self.gapLine = tmpLine
                try:
                    gapChr,chrStart,chrEnd,gene,geneStart,geneEnd = self.gapLine
                except ValueError:
                    self.gapOpen=False
                    break
            self.chrGaps[self.gapChr]=gapCands
            gapCands = dd(list)
            self.gapChr = gapChr
            gapCnt=1
...
#######################################################################################################################################################
    ...
    def gapPrint(self,gene,gapList):
        MIN_CNT=5
        myStarts=[0]; myEnds=[0]
        self.geneSeq =  "".join([ base.capitalize() for base in self.seq[gene.start-1:gene.end]])
        for spot in gene.knownJxns:
            myStarts.extend([x[0] for x in spot[1::]])
            myEnds.extend([x[1] for x in spot[0:len(spot)-1]])

        myStarts=sorted(list(set(myStarts))); myEnds=sorted(list(set(myEnds))); i=0;j=0;k=0

        for g in gapList:
            gapEnd,gapStart,gapCnt=g
            gapDist=gapStart-gapEnd
            while i<len(myEnds)-1 and  gapEnd > myEnds[i]: i+=1
            while j<len(myEnds)-1 and  gapEnd > myStarts[j]: j+=1
            while k<len(gene.exons)-1 and gapEnd > gene.exons[k][1]: k+=1
            x=j;y=k

            while x<len(myStarts)-1 and gapStart > myStarts[x]: x+=1
            while y<len(gene.exons)-1 and gapStart > gene.exons[y][1]: y+=1

            if gapEnd != myEnds[i] and gapStart != myStarts[x] and gapCnt < MIN_CNT: continue

            if gapEnd == myEnds[i]: jTYPE = "exCANON_NOVEL_"
            elif gapEnd <= gene.exons[k][1] and gapEnd >= gene.exons[k][0]: jTYPE="exNOVEL_NOVEL_"
            else:                               jTYPE="intNOVEL_NOVEL_"
            
            if gapStart == myStarts[x]: jTYPE+="exCANON"
            elif gapStart <= gene.exons[y][1] and gapStart >=gene.exons[y][0]: jTYPE+="exNOVEL"
            else:                       jTYPE+="intNOVEL"
            geneInfo = gene.name+"|"+gene.hugo+"|"+gene.type+"|"+gene.chr+"|"+gene.strand+"|JCAND_"
            endOffset,startOffset = gapEnd-gene.start+1,gapStart-gene.start
            gapSeq = (self.geneSeq[endOffset-(self.readlen-1):endOffset],self.geneSeq[endOffset:endOffset+2],self.geneSeq[startOffset-2:startOffset],self.geneSeq[startOffset:startOffset+self.readlen-1]) 

            jDATA = "".join([geneInfo,jTYPE+"_"+gapSeq[1]+"_"+gapSeq[2]+":",str(gapEnd-(self.readlen-2)),"-",str(gapEnd),"|",str(gapStart),"-",str(gapStart+(self.readlen-2))])
            jSEQ  = gapSeq[0]+gapSeq[3]
            self.gapCandPrint.write(">%s\n%s\n" % (jDATA,jSEQ))
    ...
    def printGenesOnChromosome(self,TYPE='ALL'):
        if self.FIRSTPRINT:
            ...
            if self.gapList:
                self.gapCandPrint    = open(self.prefix+'_gapCands.fa','w')
                self.gapCandPrint.write(">%s\n%s" % ("NULL_SEQUENCE","".join(["N" for i in range((self.readlen*2)-2)])))
            else:
                ...
                for name,seq in self.filters.seqs:
                    self.exonPrint.write("%s:0-%s\n%s\n" % (name,len(seq),seq))
            
        
        if self.gapList:
            if self.chr in self.chrGaps.keys():
                for gene in self.genes:
                    if self.chrGaps[self.chr][gene.name]:
                        self.gapPrint(gene,sorted(self.chrGaps[self.chr][gene.name]))
